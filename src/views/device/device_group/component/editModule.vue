...
import {listDeviceGroupTree,addDeviceGroup,updateDeviceGroup} from "@/api/device/device_group";
...
const openDialog = (row: any) => {
  ...
  // 查询组织下拉树结构
  listDeviceGroupTree().then((response: any) => {
    state.groupOptions = [];
...
const closeDialog = (row?: object) => {
  proxy.mittBus.emit("onEditDeviceGroupModule",row)
  ...
  formWrap.validate((valid: boolean) => {
    if (valid) {
      ...
      if (state.ruleForm.id != undefined && state.ruleForm.id != 0) {
        updateDeviceGroup(state.ruleForm).then((response) => {
          ElMessage.success("修改成功");
          state.loading = false;
          closeDialog(state.ruleForm); // 关闭弹窗
        });
      } else {
        addDeviceGroup(state.ruleForm).then((response) => {
