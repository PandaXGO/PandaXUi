...
import {listProductCategoryTree,addProductCategory,updateProductCategory} from "@/api/device/product_category";
...
const openDialog = (row: any) => {
  ...
  // 查询组织下拉树结构
  listProductCategoryTree().then((response: any) => {
    state.groupOptions = [];
...
const closeDialog = (row?: object) => {
  proxy.mittBus.emit("onEditProductCategoryModule",row)
  ...
  formWrap.validate((valid: boolean) => {
    if (valid) {
      ...
      if (state.ruleForm.id != undefined && state.ruleForm.id != 0) {
        updateProductCategory(state.ruleForm).then((response) => {
          ElMessage.success("修改成功");
          state.loading = false;
          closeDialog(state.ruleForm); // 关闭弹窗
        });
      } else {
        addProductCategory(state.ruleForm).then((response) => {
