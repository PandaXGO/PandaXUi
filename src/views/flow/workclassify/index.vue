    ...
    <el-card class="box-card">
      <template #header>
        <div class="card-header">
          <span class="card-header-text">工作流分类列表</span>
          <div>
            <el-button
                type="primary"
                plain
...
<script lang="ts" setup name="WorkClassify">
import {
  ref,
  reactive,
  onMounted,
...
import { listWorkClassify, delWorkClassify } from "@/api/flow/workclassify";
...
const handleQuery = () => {
  state.loading = true;
  listWorkClassify(state.queryParams).then((response:any) => {
...
const onTabelRowDel = (row: any) => {
  ...
  ElMessageBox({
    ...
    return delWorkClassify(ids).then(() => {
      handleQuery();
...
onMounted(() => {
  ...
  proxy.mittBus.on("onEditWorkClassifyModule", (res: any) => {
    handleQuery();
  });
});
// 页面卸载时
onUnmounted(() => {
  proxy.mittBus.off("onEditWorkClassifyModule");
});
</script>
