    ...
    <el-card class="box-card">
      <template #header>
        <div class="card-header">
          <span class="card-header-text">大屏分组列表</span>
          <div>
            <el-button type="primary"
                       plain
                       v-auth="'screen:group:add'"
        ...
        >
          <template #default="scope">
            <el-popover  placement="left">
              ...
              <div>
                <el-button text type="primary" v-auth="'screen:group:edit'" @click="onOpenEditModule(scope.row)">
                  <SvgIcon name="elementEdit"/>
                  修改
                </el-button>
              </div>
              <div>
                <el-button text type="primary" v-auth="'screen:group:add'" @click="onOpenAddModule(scope.row)">
                  <SvgIcon name="elementPlus"/>
                  新增
                </el-button>
              </div>
...
import {delScreenGroup, listScreenGroup} from "@/api/visual/screen_group";
...
const handleQuery = () => {
  state.loading = true;
  listScreenGroup(state.queryParams).then((response: any) => {
...
const onTabelRowDel = (row: any) => {
  ElMessageBox({
    ...
    return delScreenGroup(row.id).then(() => {
      handleQuery();
...
onMounted(() => {
  ...
  proxy.mittBus.on("onEditScreenGroupModule", (res: any) => {
    handleQuery();
  });
});
// 页面卸载时
onUnmounted(() => {
  proxy.mittBus.off("onEditScreenGroupModule");
});
</script>
